 ...
 *
 * Uses a vanilla IndexedDB API (no external library dependency since Dexie
 * isn't installed via npm yet). Provides:
 *  - saveEngagement(record)   — save a draft to the outbox
 *  - getAllDrafts()           — list all pending drafts
 *  - getDraft(localUuid)      — get one draft
 *  - deleteDraft(localUuid)   — remove after successful sync
 *  - saveMasterData(payload)  — cache campaigns/cities/woredas
 *  - getMasterData(key)       — retrieve cached data
 */
...
function openDB() {
    ...
    return new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = (e) => {
            ...
            // Outbox — one record per offline draft
            ...
            // Master data cache (campaigns, sub_cities, woredas, violation_types)
...
/**
 * Save a new draft engagement to the outbox.
 * Automatically assigns a local_uuid and timestamps.
 */
export function saveEngagement(record) {
    const draft = {
        ...
        _outbox_status:    'pending',    // pending | syncing | synced | failed
    };
