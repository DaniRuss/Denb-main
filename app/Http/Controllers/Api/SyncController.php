...
class SyncController extends Controller
{
    public function sync(Request $request)
    {
        $request->validate([
            ...
            'records.*.campaign_id' => 'required|integer|exists:campaigns,id',
            'records.*.engagement_type' => 'required|string',
            'records.*.sub_city_id'     => 'required|integer|exists:sub_cities,id',
            'records.*.woreda_id'       => 'required|integer|exists:woredas,id',
            'records.*.violation_type'  => 'required|string',
            'records.*.session_datetime' => 'required|date',
        ]);
        ...
        try {
            foreach ($request->records as $raw) {
                // Idempotency: if a record with this local_uuid already exists, skip it
                ...
                // Generate engagement code on the server
            ...
            DB::rollBack();
            Log::error('Offline sync failed: ' . $e->getMessage(), ['user' => $user->id]);
            return response()->json(['error' => 'Sync failed. Please try again.'], 500);
        }
